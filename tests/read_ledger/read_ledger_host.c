...
int32_t NaClSysReadRippleLedger(struct NaClAppThread *natp,
								uint32_t         ledgerHash) {
  if (natp!=0) {}
  printf("Inside custom test 'read ledger' syscall\n");
  printf("Ledger hash: %d\n", ledgerHash);
  fflush(stdout);
  /* Return a value that the test guest program checks for. */
  return 0;
}
...
static int32_t NaClSysReadRippleLedgerDecoder(struct NaClAppThread *natp) {
  struct NaClSysReadRippleLedgerArgs {
    uint32_t ledgerHash;
  } p;
  ...
  return NaClSysReadRippleLedger(natp, (uint32_t)  p.ledgerHash);
}
...
int main(int argc, char **argv) {
  ...
  syscall_table[TEST_SYSCALL_READ_LEDGER].handler = NaClSysReadRippleLedgerDecoder;
